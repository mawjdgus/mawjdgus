...
if not args.cls:
    ...
    # trainloader = torch.utils.data.DataLoader(trainloader, batch_size=4, sampler=ddp)
    trainloader = torch.utils.data.DataLoader(trainloader, batch_size=512,shuffle=False, num_workers=2,sampler = ddp)
    valloader = CustomDataset('/home/data/Imagenet/', False)
    valloader = torch.utils.data.DataLoader(valloader, batch_size=512, num_workers=2)
else:
    trainloader = CustomDataset('/home/data/Imagenet/')
    ddp = torch.utils.data.distributed.DistributedSampler(trainloader)
    trainloader = torch.utils.data.DataLoader(trainloader, batch_size=512//args.ngpu, shuffle=False, num_workers=16, sampler=ddp)
    valloader = CustomDataset('/home/data/Imagenet/', False)
    val_ddp = torch.utils.data.distributed.DistributedSampler(valloader)
    valloader = torch.utils.data.DataLoader(valloader, batch_size=512, num_workers=16)
...
print('Number of train dataset: ' ,len(trainloader))
print('Number of validation dataset: ' ,len(valloader))
...
if args.ngpu > 1:
    ...
    net = torch.nn.parallel.DistributedDataParallel(net, device_ids=[args.local_rank], output_device=args.local_rank, broadcast_buffers=False)
...
def train(epoch):
    ...
    for batch_idx, (inputs1,inputs2, targets1,targets2) in enumerate(trainloader):
        ...
        if not args.cls:
            ...
            cls_loss = kdloss(outputs, outputs_cls)
            # print(cls_loss)
            # cls_loss.backward()
...
def val(epoch):
    ...
    with torch.no_grad():
        for batch_idx, (inputs, targets) in enumerate(loader):
            if use_cuda:
                inputs, targets = inputs.cuda(args.local_rank), targets.cuda(args.local_rank)
